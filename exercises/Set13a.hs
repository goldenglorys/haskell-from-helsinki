...
checkNumber (f, s) = if any isDigit f || any isDigit s then Nothing else Just (f, s)
...
checkCapitals (for,sur) = if not (null for) && isUpper (head for) && not (null sur) && isUpper (head sur) then Just (for, sur) else Nothing
...
countAndLog p (x:xs) = do
  if p x
    then do
      msg (show x)
      rest <- countAndLog p xs
      return (rest + 1)
    else countAndLog p xs
...
balance accountName = BankOp $ \bank@(Bank accounts) ->
  let amount = Map.findWithDefault 0 accountName accounts
  in (amount, bank)
...
rob from to = balance from +> \amount -> withdrawOp from amount +> \withdrawn -> depositOp to withdrawn
  ...
  if s == -1
    then return ()
      ...
      then put (s + 1)
        ...
        then if s == 0 then put (-1) else put (s - 1)
        else return ()
