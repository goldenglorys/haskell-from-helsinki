...
data State = Start | HasEggs | HasEggsFlour | HasEggsSugar | ReadyToMix | ReadyToBake | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = HasEggs
step HasEggs AddFlour = HasEggsFlour
step HasEggs AddSugar = HasEggsSugar
step HasEggsFlour AddSugar = ReadyToMix
step HasEggsSugar AddFlour = ReadyToMix
...
passwordAllowed str (ContainsSome chars) = any (`elem` chars) str
passwordAllowed str (DoesNotContain chars) = not (any (`elem` chars) str)
...
data Arithmetic = Lit Integer | Op String Arithmetic Arithmetic
  deriving Show
