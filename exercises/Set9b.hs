...
prettyPrint size coords = concat [line r | r <- [1..size]]
  where
    line r = [if (r,c) `elem` coords then 'Q' else '.' | c <- [1..size]] ++ "\n"
...
danger c stack = any (\q -> sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q) stack
...
prettyPrint2 size stack = concat [line r | r <- [1..size]]
  where
    line r = [char r c | c <- [1..size]] ++ "\n"
    char r c
      | (r,c) `elem` stack = 'Q'
      | danger (r,c) stack = '#'
      | otherwise = '.'
...
fixFirst n (c:cs)
  | snd c > n = Nothing
  | danger c cs = fixFirst n (nextCol c : cs)
  | otherwise = Just (c:cs)
fixFirst _ [] = Nothing
