...
greetText name =
  let prefix = T.pack "Hello, "
      suffix = T.pack "!"
      limit = 15
  in if T.length name > limit
     then T.concat [prefix, T.take limit name, T.pack "...", suffix]
     else T.concat [prefix, name, suffix]
...
shout :: T.Text -> T.Text
shout = T.unwords . zipWith (\i w -> if even i then T.toUpper w else w) [(0::Int)..] . T.words
...
pingpong b = BL.cycle (BL.fromStrict b `BL.append` BL.fromStrict (B.reverse b))
