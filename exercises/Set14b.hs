...
balance conn account = do
  rows <- query_ conn getAllQuery :: IO [(String, Int)]
  return $ sum [amt | (acc, amt) <- rows, T.pack acc == account]
...
parseCommand [cmd, account, amountTxt]
  | cmd == T.pack "deposit" = Deposit account <$> parseInt amountTxt
  | cmd == T.pack "withdraw" = Withdraw account <$> parseInt amountTxt
