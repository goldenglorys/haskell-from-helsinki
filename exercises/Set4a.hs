...
longest :: Ord a => [[a]] -> [a]
longest xs = xs'
  where (_, _, xs') = maximum [ (length x, Down (head x), x) | x <- xs ]
...
incrementKey k = map (\(key, val) -> if key == k then (key, val + 1) else (key, val))
...
winner scores player1 player2 =
  let p1score = Map.findWithDefault 0 player1 scores
...
transfer from to amount bank
  | amount >= 0
  , Just fromBal <- Map.lookup from bank
  , fromBal >= amount
  , Map.member to bank =
