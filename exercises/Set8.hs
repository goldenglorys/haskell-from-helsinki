...
paint (Picture p1) (Shape s) (Picture p2) = Picture f
  where f coord
          | s coord   = p1 coord
          | otherwise = p2 coord
------------------------------------------------------------------------------
  ...
  apply Blur (Picture p) = Picture f
    where f (Coord x y) =
            let c1 = p (Coord x y)
                ...
                c4 = p (Coord x (y-1))
                c5 = p (Coord x (y+1))
                r = (getRed c1 + getRed c2 + getRed c3 + getRed c4 + getRed c5) `div` 5
                g = (getGreen c1 + getGreen c2 + getGreen c3 + getGreen c4 + getGreen c5) `div` 5
                b = (getBlue c1 + getBlue c2 + getBlue c3 + getBlue c4 + getBlue c5) `div` 5
            in Color r g b
------------------------------------------------------------------------------
